...
<form action="" method="post">''')       
      with open('app/templates/moreshet/edit/moreshet_edit_maintenance_mafil.html','w', encoding='utf-8-sig') as f:
        f.writelines([dphtml + '\n' + r'''
  ...
  <form method="POST">
    ...
    <button type="sumbit" name="options" value="option_delet" class="btn btn-outline-danger" onclick="fireDeletAlert()">מחיקה</button>
      ...
      if request.form.get('options') == 'option_edit':
        ...
        sys.stdout.flush()
        with open('app/db/moreshet/maintenance.csv', 'w', newline='', encoding='utf-8-sig') as file:
