...
async def moreshet_show_data_errors_mafil_Handler(request):
...
<form action="" method="post">''')  
        ...
        dphtml += data_errors.to_html(table_id="moreshet_show_data_errors_mafil", classes = "table table-see-all-errors table-hover", border=0)
        with open('app/templates/moreshet/show/moreshet_show_data_errors_mafil.html','w', encoding='utf-8-sig') as f:
            f.writelines([dphtml + '\n' + r'<br>'  + '\n' + r"</form>",r"</section>",
            r'''
<div class="container">
...
$('#moreshet_show_data_errors_mafil').DataTable();
</script>"
</body>
{% endblock %}'''])
            f.close()
        return render_template('moreshet/show/moreshet_show_data_errors_mafil.html')

    elif request.method == 'POST':
        if request.form.get("options") == 'option1':
            return redirect(url_for('moreshet_insert_error'))
        elif request.form.get("options") == 'option2':
            return redirect(url_for('moreshet_edit_data_errors_mafil'))
