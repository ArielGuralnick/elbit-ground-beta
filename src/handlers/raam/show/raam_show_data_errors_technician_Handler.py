...
async def raam_show_data_errors_technician_Handler(request):
    if request.method == 'GET':      
        data_errors = pd.read_csv('app/db/raam/data_errors.csv')
        dphtml = (r'''
{% extends 'layout.html' %}
{% block content %}
...
<form action="" method="post">''')  
        ...
        dphtml += data_errors.to_html(table_id="show_data_errors_technician", classes = "table table-see-all-errors table-hover", border=0)
        with open('app/templates/raam/show/raam_show_data_errors_technician.html','w', encoding='utf-8-sig') as f:
            f.writelines([dphtml + '\n' + r'<br>' +'\n' +
           r'''
<div class="container">
...
{% endblock %}'''])
            f.close()
        return render_template('raam/show/raam_show_data_errors_technician.html')
   
    elif request.method == 'POST':
        if request.form.get("options") == 'option1':
            return redirect(url_for('raam_insert_error'))
        elif request.form.get("options") == 'option2':
            return redirect(url_for('raam_edit_data_errors'))
        elif request.form.get("options") == 'option3':
            return send_file('db/raam/data_errors.csv',
            mimetype='text/csv',attachment_filename='דוח תקלות רעם.csv',
            as_attachment=True)
