...
async def raam_show_warehouse_inventory_Handler(request):

    if request.method == 'GET':
        data = pd.read_csv('app/db/raam/warehouse_inventory.csv')
        dphtml = (r'''
{% extends 'layout.html' %}
{% block content %}
...
<section id="show_data_errors" dir="rtl" lang="he">''')
        ...
        with open('app/templates/raam/show/raam_show_warehouse_inventory.html','w', encoding='utf-8-sig') as f:
            f.writelines([dphtml + '\n' + r'<br>'  + '\n' + r'</section>' + '\n' + 
...
{% endblock %}'''])
            f.close()
        return render_template('raam/show/raam_show_warehouse_inventory.html')
        ...
        if request.form.get("options") == 'option_add':
            ...
            if type_of_item == "" or model == "" or quantity =="" :
                ...
                sys.stdout.flush()
                with open('app/db/raam/warehouse_inventory.csv', 'a', newline='', encoding='utf-8-sig') as file:
            ...
            src_upload_file_path = "app/db/raam/warehouse_inventory.csv"
            ...
            return redirect(url_for('raam_show_warehouse_inventory'))

        elif request.form.get("options") == 'option_edit':
            return redirect(url_for('raam_edit_warehouse_inventory'))
        
        elif request.form.get("options") == 'option_open_csv':
            return send_file('db/raam/warehouse_inventory.csv',
            mimetype='text/csv',attachment_filename='מחסן רעם.csv',
            as_attachment=True)
