...
async def raam_show_maintenance_technician_mafil_Handler(request):

    if request.method == 'GET':
        data = pd.read_csv('app/db/raam/maintenance.csv')
        dphtml = (r'''
{% extends 'layout.html' %}
{% block content %}
...
<section id="show_data_errors" dir="rtl" lang="he">''')
        ...
        with open('app/templates/raam/show/raam_show_maintenance_technician_mafil.html','w', encoding='utf-8-sig') as f:
            f.writelines([dphtml + '\n' + r'<br>' + '\n' + r"</form>" + '\n' + r"</section>" + '\n' +
...
{% endblock %}'''])
            f.close()
        return render_template('raam/show/raam_show_maintenance_technician_mafil.html')
        ...
        if request.form.get("options") == 'option_add':
            ...
            if date_upload == "" or disparity == "" :
                ...
                sys.stdout.flush()
                with open('app/db/raam/maintenance.csv', 'a', newline='', encoding='utf-8-sig') as file:
            ...
            src_upload_file_path = "app/db/raam/maintenance.csv"
            ...
            return redirect(url_for('raam_show_maintenance_technician_mafil'))

        elif request.form.get("options") == 'option_edit':
            return redirect(url_for('raam_edit_maintenance_technician_mafil'))
