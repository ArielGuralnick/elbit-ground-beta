...
async def driving_show_data_activity_Handler(request):
    if request.method == 'GET':       
        data_activity = pd.read_csv('elbit-ground-beta/app/db/driving/data_activity.csv')
...
<div class="row">
    ...
    <form method="POST">
        ...
        with open('elbit-ground-beta/app/templates/driving/show/driving_show_data_activity.html','w', encoding='utf-8-sig') as f:
            f.writelines([dphtml + '\n' + r'<br>' + '\n' +
            r"</form>" + '\n' + r"</section>" + '\n' + 
            r'<script type="text/javascript">' + '\n' + r"$('#show_activity').DataTable();" + 
            '\n' + r"</script>" + r"</body>" + '\n' + r"{% endblock %}"])
            f.close()
        return render_template('driving/show/driving_show_data_activity.html')
...
<div class="row">
    ...
    <form method="POST">
        <button type="sumbit" name="options" value="option2" class="btn btn-outline-secondary">פתיחת דוח באקסל</button>
            ...
            return send_file('db/driving/data_activity.csv',
            mimetype='text/csv',attachment_filename='דוח פעילות מאמני נהיגה.csv',
            as_attachment=True)