...
<form action="" method="post">''')       
      with open('app/templates/driving/edit/driving_edit_maintenance_mafil.html','w', encoding='utf-8-sig') as f:
        f.writelines([dphtml + '\n' + r'''
  ...
  <form method="POST">
    ...
    <button type="sumbit" name="options" value="option_delet" class="btn btn-outline-danger" onclick="fireDeletAlert()">מחיקה</button>
      ...
      if request.form.get('options') == 'option_edit':
        ...
        sys.stdout.flush()
        with open('app/db/driving/maintenance.csv', 'w', newline='', encoding='utf-8-sig') as file:
      ...
      if request.form.get('options') == 'option_delet':
        ...
        print("CD=", current_cd_path)
        sys.stdout.flush()
        with open('app/db/driving/8.csv', 'w', newline='', encoding='utf-8-sig') as file:
            data.to_csv(file, index=False, na_rep='N/A',header=file.tell()==0, encoding='utf-8-sig')
