...
async def driving_edit_data_errors_Handler(request):
    if request.method == 'GET':       
      data = pd.read_csv('app/db/driving/data_errors.csv')
      error = data.index
...
<form action="" method="post">''')       
      with open('app/templates/driving/edit/driving_edit_data_errors.html','w', encoding='utf-8-sig') as f:
        f.writelines([dphtml + '\n' + r'''
...
{% endblock %}'''])
      ...
      if request.form.get('options') == 'option_edit':
        ...
        with open('app/db/driving/data_errors.csv', 'w', newline='', encoding='utf-8-sig') as file:
